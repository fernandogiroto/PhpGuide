...
if($_GET['codigo']){
    ...
    if($stmt->execute()){
        $resultdo = $stmt->get_result();
        if($resultdo->num_rows>0){
            $dados = $resultdo->fetch_assoc();
        }
    }
}
var_dump($dados);
...
if(count($_POST) > 0) {
    ...
    if(!count($erros)) {
        $sql = "UPDATE cadastro 
        SET nome = ?, email = ?, site = ?, filhos = ?, salario = ?
        WHERE id = ?";
...
<div class="form-group row">
    <div class="col-sm-10">
        <input class="form-control" type="number" name="codigo" value="<?php $_GET['codigo'] ?>" placeholder="Codigo para colsulta">
    </div>
   <div class="col-sm-2">
